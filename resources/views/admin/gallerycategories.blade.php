  ...
  <div class="container-xl px-4 mt-n10">
    <div class="card mb-4">
      <div class="card-header text-uppercase">
        <span class="ml-5">
            <a href="javascript:void(0)" class="ms-2 addGalleryCategory" data-bs-toggle="modal"
                    ...
                    <tbody>
                        @foreach ($gallerycategories as $row)
                            <tr>
                                ...
                                <td>
                                    {{ $row->name }} <label style="color: red">({{ $row->galleries->count() }})</label>
                                </td>
                                <td>
                                    <form action="/admin/gallerycategory/{{ $row->id }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <a href="javascript:void(0)" class="editGalleryCategory btn btn-datatable btn-icon btn-primary"
                                            data-id="{{ $row->id }}" data-bs-toggle="modal"
                                            data-bs-target="#galleryCategoryModal"><i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <button {{ $row->galleries->count() != 0 ? 'disabled' : '' }}
